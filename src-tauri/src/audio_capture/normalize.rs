...
pub(crate) struct PcmNormalizer {
    input_rate: u32,
    channels: usize,
    input_frames: u64,
    output_samples: u64,
...
impl PcmNormalizer {
    pub(crate) fn new(
        ...
        let mut resampler = if input_rate == OUTPUT_SAMPLE_RATE {
            None
        } else {
            Some(
                Async::<f32>::new_sinc(
                    f64::from(OUTPUT_SAMPLE_RATE) / f64::from(input_rate),
                    1.0,
                ...
                .map_err(|error| AudioCaptureError::Resampler(error.to_string()))?,
            )
        };
    ...
    pub(crate) fn push_interleaved(
        &mut self,
        samples: &[f32],
    ) -> Result<Vec<AudioFrame>, AudioCaptureError> {
        if !samples.len().is_multiple_of(self.channels) {
            return Err(AudioCaptureError::UnalignedSamples {
                samples: samples.len(),
    ...
    pub(crate) fn finish(&mut self) -> Result<Vec<AudioFrame>, AudioCaptureError> {
        self.process_complete_chunks()?;
        ...
        while self.output_samples + (self.pending_output.len() as u64) < expected_samples
            && self.resampler.is_some()
        {
            self.input_buffer[0].fill(0.0);
            self.process_resampler(Some(0))?;
        }
        let remaining = expected_samples.saturating_sub(self.output_samples) as usize;
        self.pending_output.truncate(remaining);
        Ok(self.take_complete_frames(true))
    }

    fn process_complete_chunks(&mut self) -> Result<(), AudioCaptureError> {
        if self.resampler.is_none() {
            self.pending_output.extend(self.pending_mono.drain(..));
            return Ok(());
        }
    ...
    fn process_resampler(&mut self, partial_len: Option<usize>) -> Result<(), AudioCaptureError> {
    ...
    fn take_complete_frames(&mut self, include_partial: bool) -> Vec<AudioFrame> {
        ...
        while self.pending_output.len() >= OUTPUT_FRAME_SAMPLES
            || (include_partial && !self.pending_output.is_empty())
        {
            let count = self.pending_output.len().min(OUTPUT_FRAME_SAMPLES);
            let samples: Vec<f32> = self.pending_output.drain(..count).collect();
            let start_sample = self.next_start_sample;
            self.next_start_sample += samples.len() as u64;
            self.output_samples += samples.len() as u64;
...
mod tests {
    ...
    fn normalize(rate: u32, channels: u16, seconds: usize) -> Vec<AudioFrame> {
        let frames = rate as usize * seconds;
        let mut input = Vec::with_capacity(frames * usize::from(channels));
        for index in 0..frames {
            let value = (index as f32 * 0.01).sin();
            input.extend(std::iter::repeat_n(value, usize::from(channels)));
        }
        let mut normalizer = PcmNormalizer::new(rate, channels, 37).unwrap();
        let mut output = Vec::new();
        for chunk in input.chunks(777 * usize::from(channels)) {
    ...
    #[test]
    fn normalizes_common_rates_and_channel_counts() {
        for (rate, channels) in [(44_100, 1), (48_000, 2), (96_000, 6), (16_000, 2)] {
            let output = normalize(rate, channels, 1);
            assert_eq!(
                output
                    .iter()
    ...
    #[test]
    fn emits_a_single_short_final_frame() {
        let mut normalizer = PcmNormalizer::new(16_000, 1, 0).unwrap();
        assert!(normalizer.push_interleaved(&vec![0.0; 700]).unwrap().len() == 1);
    ...
    fn rejects_partial_interleaved_frames() {
        ...
        assert!(matches!(
            normalizer.push_interleaved(&[0.0, 1.0, 2.0]),
            Err(AudioCaptureError::UnalignedSamples { .. })
        ));
    }
}
