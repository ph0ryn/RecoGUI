...
マイク、Mac全体のデスクトップ音声、または音声ファイルを入力とし、確定した文字起こしを
SQLite に保存してから画面へ表示する。
...
Windows、Linux、Intel Mac、macOS 14.0および14.1、他言語へのローカライズは対象外とする。

## 機能要件

### 文字起こし

- 入力種別は`file`、`microphone`、`systemAudio`の三つとする。
- 音声ファイルはPythonが直接読み、マイクとデスクトップ音声はRustが取得したPCMをPythonへ渡す。
- デスクトップ音声は特定アプリの選択式ではなくMac全体の出力を対象とし、RecoGUI自身の出力だけを
  除外する。取得中もユーザーへの通常の音声出力を継続する。
- マイク選択はOSから取得した永続的なdevice IDを保存し、新規開始とResumeの直前にRustが解決する。
- 保存したdevice IDのマイクが利用できない場合は、別の入力デバイスやシステム既定へfallbackしない。
- デスクトップ音声は選択デバイスを保存せず、開始時点のMac全体の出力を取得する。
- 入力を 16 kHz mono に正規化し、sample index を時刻の正本とする。
- 同時に実行できるセッションは一つとする。
- 実行中セッションをpauseし、同じセッションとしてresumeできる。
- ライブ入力の新規開始では、必要なOS権限とcapture可否をRustが事前確認する。拒否、キャンセル、
  事前確認失敗時はsession rowを作成せず、具体的な理由を画面へ返す。
- 新規開始またはResumeではsessionを`preparing`として保存し、modelの読込とRust captureの開始が
  両方成功した後だけ`running`へ移行する。
...
- マイクとデスクトップ音声はresume時点から同じmedia時間軸へ追記し、pause中の実時間を
  timestampへ含めない。音声ファイルは保存した処理位置から再開する。
- Resumeはsessionに保存したrepository IDとrevisionを使用し、現在の既定modelや別revisionへfallbackしない。
- Resume時のmodel読込またはライブcapture開始失敗は再開位置を維持して`paused`へ戻し、
  新規開始時のmodel読込失敗は`failed`とする。
- Stop は処理待ちの音声を可能な範囲で完了してから終了する。
- pausedのライブセッションはresumeせず、その場でStopして履歴を確定できる。
- 明示的にpauseしたライブセッションだけをresume可能とする。録音中のデバイス切断、PCM欠落、
  バッファoverflow、音声transport切断、capture失敗による`failed`はresume不可とし、新しいsessionを開始する。
- 音声ファイルの`failed`は最後に保存したsegment終端から同じsessionへ再試行できる。
- ASR modelとworkerは処理中だけmemoryに保持し、起動時、model選択時、idle、paused中には保持しない。

### 音声取得

- Rustは一度に一つのライブcaptureだけを所有し、マイクとデスクトップ音声を同時取得しない。
- ライブ音声は有界bufferを通してPythonへ渡し、欠落または順序不整合を成功扱いしない。
- Rustの音声callbackはblocking I/O、memory確保、log出力を行わない。
- stop、pause、sleep、sidecar終了、application終了ではcaptureを停止し、OS resourceを残さない。
- sleepまたは出力デバイス変更後にライブcaptureを自動再開しない。
- DRMなどで保護されたコンテンツはデスクトップ音声として取得できず、無音になる場合がある。
...
- セッションは権限の事前確認後、実際の音声処理を始める前に SQLite へ作成する。
- 確定セグメントは SQLite への保存に成功してから画面へ表示する。
- 停止、失敗、異常終了時も保存済みの部分を履歴に残す。音声ファイルの失敗時は最後に保存した
  segmentの終端をcheckpointとし、同じsessionへ続きから再試行できるようにする。
...
- Rust と Python は制御用のversion付きNDJSONと、ライブPCM専用のversion付きbinary streamで通信し、
  不正、過大、不整合、欠落したmessageを拒否する。
...
- マイクとデスクトップ音声のmix
- デスクトップ音声を取得するアプリの選択
- 録音中のデバイス切替とcaptureの自動再接続
- macOS 14.0および14.1向けの代替capture方式
- 旧engine protocolおよび旧音声入力実装との互換性
- 文字起こしの編集
