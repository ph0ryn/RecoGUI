...
class LocalAsrTranscriptionService:
  ...
  def _generate(self, model: Any, segment: SpeechSegment, max_tokens: int) -> Any:
    ...
    try:
      ...
      raise AsrRuntimeError(f"Local ASR transcription failed: {exc}") from exc
  ...
  def load_model(self) -> Any:
    ...
    with self._load_lock:
      ...
      try:
        ...
        raise AsrRuntimeError(f"Could not load ASR model from {identity}: {exc}") from exc
...
def _result_language(result: object, configured_language: str | None) -> str:
  ...
  raise AsrRuntimeError("The ASR model did not return a detected language")
...
def _load_mlx_audio_model(model_path: str, revision: str | None) -> Any:
  try:
    ...
    raise AsrRuntimeError("mlx-audio is not installed. Prepare the RecoGUI worker environment first.") from exc
  if revision is None:
